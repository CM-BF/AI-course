...
class VData(object):
    ...
    def __init__(self, k, gap):
        ...
        for i in range(len(self.coord)):
            tmpcoord[i % 5].append(self.coord[i])

        self.coord = []
        for i in range(5):
            if i != k:
                self.coord += tmpcoord[i]
            else:
                self.data['test'] = iter(tmpcoord[k])
...
def SVMmain(verify, data, sigma, C):
    ...
    def multiclassSVM():
        ...
        # 构造多个SVM
        result = {}
        print('Begin train', file=file)
        for SVMclass in TP.keys():
            result[SVMclass] = softSVM(SVMclass, sigma, C)
            print('finished SVM: SVM' + SVMclass, file=file)

        print('Begin test', file=file)
        # 开始测试
        next(data.data['test'])
        for item in data.data['test']:
            sample = item
            if not verify:
                sample = data.transformer(sample)
            ...
            if ypred == sample['class']:
                ...
                print(total_items + 1, ypred, sample['class'], file=file)
            for key in TP.keys():
                if key == ypred:
                    if key == sample['class']:
                        TP[key] += 1
            ...
            if total_items % 1000 == 0:
                ...
                print('total:', total_items, 'Accuracy:', accuracy, 'Macro F1:', MacroF1, 'Micro F1:', MicroF1, file=file)
...
if __name__ == '__main__':
    ...
    gap = 4
    k_fold_matrix = np.zeros((10, 5))
    for k in range(10):
        for fold in range(5):
            start = time.clock()
            check = VData(fold, gap)
            k_fold_matrix[k, fold] = SVMmain(True, check, sigma[k], C[k])
            print('finish:', k, fold, 'in', time.clock() - start, 's', file=file)
            print('finish:', k, fold, 'in', time.clock() - start, 's')
    print(k_fold_matrix, file=file)
    a = np.sum(k_fold_matrix, axis=1) / 5
    max = 0
    best = None
    for i in range(10):
        if a[i] > max:
            max = a[i]
            best = i
    print('---------------------best=', best, file=file)

    start = time.clock()
    check = Data(gap)
    SVMmain(False, check, sigma[best], C[best])
    print('finished train in', time.clock() - start, 's', file=file)
    print('finished all in', time.clock() - begin, 's', file=file)
    file.close()
